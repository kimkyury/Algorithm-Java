...
class Solution {
	...
	private static boolean run(BufferedReader br) throws Exception {
		int q = Integer.parseInt(br.readLine()); // 18
		...
		for (int i = 0; i < q; ++i) {
			...
			// System.out.println("각 테스트 내 함수 수행시간: " + (endTime - startTime));
		}
	...
	public static void main(String[] args) throws Exception {
		...
		StringTokenizer st = new StringTokenizer(br.readLine(), " "); // 25 100

		TC = Integer.parseInt(st.nextToken()); // 25
		MARK = Integer.parseInt(st.nextToken()); // 100

		for (int testcase = 1; testcase <= TC; ++testcase) {
			long startTime = System.currentTimeMillis();
			int score = run(br) ? MARK : 0;
			long endTime = System.currentTimeMillis();
			System.out.println("함수 수행시간: " + (endTime - startTime));
